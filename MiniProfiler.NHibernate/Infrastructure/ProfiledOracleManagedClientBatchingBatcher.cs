...
{
    public class ProfiledOracleManagedClientBatchingBatcher: AbstractBatcher {
        ...
        public ProfiledOracleManagedClientBatchingBatcher(ConnectionManager connectionManager, IInterceptor interceptor)
        ...
        private void SetArrayBindCount(int arraySize)
        {
            ...
            if (_currentBatch is ProfiledGenericDbCommand<OracleCommand>)
            {
                v_command = ((ProfiledGenericDbCommand<OracleCommand>)_currentBatch).InternalCommand;
            }
            else
            {
                v_command = _currentBatch;
            }

            var objType = v_command.GetType();
            var propInfo = objType.GetProperty("ArrayBindCount");
            propInfo.SetValue(v_command, arraySize, null);
        }
